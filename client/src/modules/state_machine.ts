...
export class StateMachine<StateEnum, EventEnum> {
    ...
    constructor(cfg: StateMachineConfig<StateEnum, EventEnum>) {
        ...
        for (let i = 0; i < cfg.events.length; i += 1) {
            ...
            let froms: any =
                cfg.events[i].from instanceof Array
                    ? cfg.events[i].from
                    : [cfg.events[i].from];

            for (let j = 0; j < froms.length; j += 1) {
                this.eventMap["" + froms[j]] =
                    this.eventMap["" + froms[j]] || {};
                this.eventMap["" + froms[j]]["" + cfg.events[i].name] =
                    cfg.events[i].to;
            }
        }
    }
    ...
    onBefore(event: EventEnum, cb: Function) {
        this.eventCb.before["" + event] = cb;
    }

    onAfter(event: EventEnum, cb: Function) {
        this.eventCb.after["" + event] = cb;
    }

    onEnter(state: StateEnum, cb: Function) {
        this.stateCb.enter["" + state] = cb;
    }

    onLeave(state: StateEnum, cb: Function) {
        this.stateCb.leave["" + state] = cb;
    }
